...
from faiss_store import FaissStore, build_faiss_store
...
# PPTX processing (optionnel)
PPTX_AVAILABLE = False
try:
    from pptx_processing import (
        extract_text_from_pptx,
        process_pptx_with_ocr,
        is_pptx_available,
        is_pptx_ocr_available,
    )
    PPTX_AVAILABLE = is_pptx_available()
except ImportError:
    logger.debug("[PIPELINE] pptx_processing non disponible - support PPTX désactivé")
...
try:
    from semantic_chunking import semantic_chunk, AdaptiveSemanticChunker
...
class OptimizedIngestionPipeline:
    ...
    def _print_ocr_diagnostic(self):
        ...
        if LLM_OCR_AVAILABLE:
            try:
                from llm_ocr import DALLEM_CONFIG_AVAILABLE
                print(f"  DALLEM_CONFIG_AVAILABLE: {DALLEM_CONFIG_AVAILABLE}")
                if DALLEM_CONFIG_AVAILABLE:
                    from models_utils import DALLEM_API_BASE, DALLEM_API_KEY
    ...
    def extract_attachments(
        ...
        for i, file_info in enumerate(file_infos):
            ...
            if file_info.local_path and file_info.extension == ".pdf":
                pdf_idx = len([f for f in file_infos[:i+1] if f.extension == ".pdf"])
    ...
    def batch_embed_and_insert(
        ...
        # Libération mémoire après embeddings
        if self.aggressive_gc:
            gc.collect()
