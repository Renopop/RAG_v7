...
def get_pdf_page_count(pdf_path: str, log=None) -> int:
    ...
    Utilise pdfplumber comme source principale (plus fiable),
    avec fallback sur PyMuPDF si pdfplumber échoue.
    """
    _log = log or logger

    # 1. Essayer pdfplumber en premier (plus fiable pour le compte de pages)
    pdfplumber_count = None
    try:
        import pdfplumber
        with pdfplumber.open(pdf_path) as pdf:
            pdfplumber_count = len(pdf.pages)
            _log.debug(f"[LLM-OCR] pdfplumber compte: {pdfplumber_count} pages")
    except ImportError:
        _log.debug("[LLM-OCR] pdfplumber non disponible")
    except Exception as e:
        _log.debug(f"[LLM-OCR] pdfplumber erreur: {e}")

    # 2. Obtenir le compte PyMuPDF pour comparaison
    try:
        import pymupdf as fitz
    except ImportError:
        try:
            ...
            _log.error("[LLM-OCR] PyMuPDF non installé")
            return pdfplumber_count or 0

    doc = None
    pymupdf_count = 0
    try:
        doc = fitz.open(pdf_path)
        pymupdf_count = len(doc)
        _log.debug(f"[LLM-OCR] PyMuPDF compte: {pymupdf_count} pages")
    except Exception as e:
    ...
    # 3. Comparer et choisir la valeur fiable
    if pdfplumber_count is not None:
        if pymupdf_count != pdfplumber_count:
            _log.warning(
                f"[LLM-OCR] ⚠️ Incohérence nombre de pages! "
                f"PyMuPDF: {pymupdf_count}, pdfplumber: {pdfplumber_count}"
            )
            print(f"\n⚠️  ATTENTION: Incohérence détectée dans le PDF!")
            print(f"   • PyMuPDF voit: {pymupdf_count} pages")
            print(f"   • pdfplumber voit: {pdfplumber_count} pages")
            print(f"   → Utilisation de {pdfplumber_count} pages (pdfplumber plus fiable)")
            print(f"   (Le PDF peut contenir des structures internes problématiques)\n")
        return pdfplumber_count

    return pymupdf_count
...
def ocr_image_with_llm(
    ...
    Args:
        ...
        base_delay: Délai de base pour backoff exponentiel en secondes (défaut: 2.0)
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Erreurs retriables: 502, 503, 504, timeout, connection errors
            is_retriable = (
                status_code in (502, 503, 504, 429) or
            ...
            if attempt < max_retries - 1 and is_retriable:
                _log.debug(
                    f"[LLM-OCR] Erreur OCR (tentative {attempt + 1}/{max_retries}): "
                    f"{type(e).__name__} {status_code or ''} - retry dans {delay:.1f}s"
                )
                time.sleep(delay)
            else:
    ...
    _log.error(f"[LLM-OCR] Échec après {max_retries} tentatives: {last_error}")
    return "", 0.0
